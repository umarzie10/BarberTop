...
export default function BarberStatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const activeClients = new Set(
      appts.filter((a) => a.client_id).map((a) => a.client_id),
    ).size;

    // by service
    const svcMap: Record<string, number> = {};
    completed.forEach((a) => {
      const n = a.services?.name || "Noma'lum";
    ...
    const dailyArr = Object.entries(daily).map(([date, amount]) => ({
      date,
      amount,
    }));
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <PageHeader
        title={t("barber.stats.title")}
        subtitle={t("barber.stats.subtitle")}
      />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat
          label={t("barber.stats.totalBookings")}
          value={stats.totalAppts}
          hint={`${stats.completed} ${t("barber.stats.completed")}`}
        />
        <Stat
          label={t("barber.stats.revenue")}
          value={`${(stats.totalRevenue / 1000).toFixed(0)}${t("barber.stats.revenueUnit")}`}
        />
        <Stat
          label={t("barber.stats.activeClients")}
          value={stats.activeClients}
        />
        <Stat
          label={t("barber.stats.rating")}
          value={rating.toFixed(1)}
          hint={`${reviewCount} ${t("barber.stats.reviews")}`}
        />
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          <Card>
            <h3 className="text-sm font-semibold mb-4">
              {t("barber.stats.dailyRevenueTitle")}
            </h3>
            <div className="h-64">
              <ResponsiveContainer>
          ...
          <Card>
            <h3 className="text-sm font-semibold mb-4">
              {t("barber.stats.topServicesTitle")}
            </h3>
