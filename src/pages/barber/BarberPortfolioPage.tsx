...
export default function BarberPortfolioPage() {
  ...
  const handleUpload = async (file: File, caption: string = "") => {
    ...
    try {
      if (mode === "before_after") {
        if (!beforeFile) {
          setUploading(false);
          return toast.error(t("barber.portfolio.beforeRequired"));
        }
      ...
      toast.success(t("barber.portfolio.uploaded"));
      load(barberId);
    } catch (e: any) {
  ...
  const del = async (id: string) => {
    if (!confirm(t("barber.portfolio.confirmDelete"))) return;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader
        title={t("barber.portfolio.title")}
        subtitle={t("barber.portfolio.subtitle")}
      />
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {(["image", "video", "before_after"] as Mode[]).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`px-3 py-1.5 text-xs rounded-md ${mode === m ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
            >
              {m === "image"
                ? t("barber.portfolio.mode.image")
                : m === "video"
                  ? t("barber.portfolio.mode.video")
                  : t("barber.portfolio.mode.beforeAfter")}
            </button>
          ))}
        </div>
        ...
        {mode === "before_after" ? (
          <div className="flex flex-wrap gap-2 items-center">
            <label className="px-3 py-2 text-xs bg-secondary rounded-md cursor-pointer flex items-center gap-1.5">
              ...
              {beforeFile
                ? `${t("barber.portfolio.before")}: ${beforeFile.name}`
                : t("barber.portfolio.beforeUpload")}
              <input
                type="file"
                accept="image/*"
                hidden
            ...
            <label
              className={`px-3 py-2 text-xs rounded-md flex items-center gap-1.5 ${beforeFile ? "bg-primary text-primary-foreground cursor-pointer" : "bg-muted text-muted-foreground cursor-not-allowed"}`}
            >
              <Upload className="w-3.5 h-3.5" />{" "}
              {t("barber.portfolio.afterUpload")}
              <input
                type="file"
                accept="image/*"
                hidden
          ...
          <label className="inline-flex items-center gap-1.5 px-3 py-2 text-xs bg-primary text-primary-foreground rounded-md cursor-pointer">
            ...
            {uploading
              ? t("common.loading")
              : `${mode === "video" ? t("barber.portfolio.mode.video") : t("barber.portfolio.mode.image")} ${t("barber.portfolio.upload")}`}
            <input
              type="file"
      ...
      {!items.length ? (
        <Empty text={t("barber.portfolio.empty")} />
      ) : (
          ...
          {items.map((p) => (
            ...
            >
              {p.media_type === "video" ? (
                ...
                />
              ) : p.media_type === "before_after" &&
                p.before_url &&
                p.after_url ? (
                <div className="grid grid-cols-2 w-full h-full">
                  <img
                    src={p.before_url}
                    alt={t("barber.portfolio.before")}
                    className="w-full h-full object-cover"
                  />
                  <img
                    src={p.after_url}
                    alt={t("barber.portfolio.after")}
                    className="w-full h-full object-cover"
                  />
                </div>
              ) : (
              ...
              <div className="absolute top-1 left-1 px-1.5 py-0.5 text-[10px] bg-black/60 text-white rounded flex items-center gap-1">
                {p.media_type === "video" ? (
                  ...
                  t("barber.portfolio.beforeAfterBadge")
                ) : (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-white text-xs mb-1">
                    {t("barber.portfolio.before")}
                  </p>
                  <img
                    src={lightbox.before_url}
                    className="rounded-lg max-h-[70vh]"
                  />
                </div>
                <div>
                  <p className="text-white text-xs mb-1">
                    {t("barber.portfolio.after")}
                  </p>
                  <img
                    src={lightbox.after_url}
                    className="rounded-lg max-h-[70vh]"
                  />
                </div>
              </div>
