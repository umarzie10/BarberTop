...
export default function Barbers() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!form.email || !form.password)
      return toast.error("Email va parol kiriting");
    setCreating(true);
    ...
    setCreating(false);
    if (error || data?.error)
      return toast.error(error?.message || data?.error || "Xato");
    toast.success("Barber yaratildi");
    setShow(false);
  ...
  const filtered = useMemo(() => {
    ...
    if (minExp > 0)
      list = list.filter((b) => (b.experience_years || 0) >= minExp);
    if (gender)
      list = list.filter((b) => b.gender === gender || b.gender === "any");

    if (chips.has("pro"))
      list = list.filter(
        (b) => plans[b.user_id] === "pro" || plans[b.user_id] === "vip",
      );
    ...
    if (myLoc) {
      list = list.map((b) => {
        if (b.latitude != null && b.longitude != null) {
          return {
            ...b,
            _dist: distanceKm(myLoc.lat, myLoc.lng, b.latitude, b.longitude),
          };
        }
        return { ...b, _dist: null };
      });
      if (radiusKm > 0)
        list = list.filter((b) => b._dist != null && b._dist <= radiusKm);
    }
    ...
    const sorted = [...list].sort((a, b) => {
      ...
      const tierRank = (uid: string) =>
        plans[uid] === "vip" ? 2 : plans[uid] === "pro" ? 1 : 0;
      const tierDiff = tierRank(b.user_id) - tierRank(a.user_id);
      if (tierDiff !== 0 && sort === "rating")
        return tierDiff || Number(b.rating) - Number(a.rating);
      switch (sort) {
        case "rating":
    ...
    <div className="p-6 max-w-6xl mx-auto">
        ...
        action={
          isAdmin ? (
            <button
              ...
              className="px-3 py-2 text-xs bg-primary text-primary-foreground rounded-md hover:opacity-90 flex items-center gap-1.5"
            >
      ...
      <div className="mb-3 flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 text-xs border border-border rounded-md bg-background hover:bg-muted flex items-center gap-1.5"
        >
      ...
      <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
        ...
        {(chips.size > 0 ||
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs rounded-full border border-destructive/40 text-destructive hover:bg-destructive/10 flex items-center gap-1 whitespace-nowrap"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-2 py-1.5 text-sm border border-border rounded-md bg-background disabled:opacity-50"
              >
                <option value="">{t("filter.all")}</option>
                {districts.map((d) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((b) => {
            ...
            return (
              <Card
                key={b.id}
                className={`relative cursor-pointer hover:border-primary forge-transition ${tier === "vip" ? "ring-2 ring-yellow-400/50" : ""}`}
              >
                {tier && tier !== "free" && (
                  <span
                    className={`absolute top-3 left-3 z-10 text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${tier === "vip" ? "bg-yellow-400 text-black" : "bg-primary text-primary-foreground"}`}
                  >
                    <Crown className="w-3 h-3" /> {t(`plans.tier.${tier}`)}
                  </span>
                )}
                {isAdmin && (
                  <button
                    ...
                    className="absolute top-3 right-3 p-1 hover:bg-destructive/10 text-destructive rounded z-10"
                  >
                  ...
                  <div className="flex items-center gap-2 mt-3 text-xs flex-wrap">
                    ...
                    {b.experience_years > 0 && (
                      <span className="text-muted-foreground">
                        · {b.experience_years} {t("common.year")}
                      </span>
                    )}
                    {b._dist != null && (
                      <span className="text-primary font-medium">
                        · 📍 {b._dist.toFixed(1)} km
                      </span>
                    )}
                    {b.district && (
                      <span className="text-muted-foreground">
                    ...
                    {b.fast_response && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-yellow-500/10 text-yellow-600 rounded">
                        ⚡
                      </span>
                    )}
                  </div>
