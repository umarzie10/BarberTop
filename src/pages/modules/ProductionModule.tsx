...
import { Plus, Search, Factory, Package, Layers, CheckCircle, MoreHorizontal } from "lucide-react";
...
const ProductionModule = () => {
  ...
  const [form, setForm] = useState({ name: "", product: "", quantity: "", deadline: "" });

  const l = {
    title: { uz: "Ishlab chiqarish", ru: "Производство", en: "Production" },
    subtitle: { uz: "Buyurtmalar, jarayon va ombor", ru: "Заказы, процесс и склад", en: "Orders, process and warehouse" },
    ...
    completed: { uz: "Tayyor", ru: "Готово", en: "Completed" },
  ...
  const handleAdd = async () => {
    ...
    await supabase.from("deals").insert({ name: form.name, company: form.product || "—", amount: Number(form.quantity) || 0, contact_name: form.deadline, stage: "lead", user_id: user.id });
    setForm({ name: "", product: "", quantity: "", deadline: "" });
    setShowAdd(false);
    fetchData();
  };

  const filtered = orders.filter(i => i.name.toLowerCase().includes(search.toLowerCase()));
  const inProgress = orders.filter(i => !["won","lost"].includes(i.stage)).length;
  const completed = orders.filter(i => i.stage === "won").length;
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[{ label: t("total"), value: orders.length, icon: Factory }, { label: t("inProgress"), value: inProgress, icon: Layers }, { label: t("completed"), value: completed, icon: CheckCircle }].map(s => (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-lg forge-shadow-sm overflow-hidden">
        <table className="w-full"><thead><tr className="border-b border-border">
          ...
          <th className="text-left text-xs font-medium text-muted-foreground px-5 py-3">{t("product")}</th>
          <th className="text-left text-xs font-medium text-muted-foreground px-5 py-3">{t("quantity")}</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onClick={e => e.stopPropagation()} className="bg-card border border-border rounded-lg forge-shadow-md w-full max-w-md p-5">
            ...
            <div className="space-y-3">
              {([["name","name"],["product","product"],["quantity","quantity"],["deadline","deadline"]] as const).map(([k,f])=>(
