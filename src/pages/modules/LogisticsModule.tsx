...
import { Plus, Search, Truck, Package, MapPin, MoreHorizontal, Clock } from "lucide-react";
...
const LogisticsModule = () => {
  ...
  const [form, setForm] = useState({ name: "", destination: "", driver: "", weight: "" });

  const l = {
    title: { uz: "Logistika", ru: "Логистика", en: "Logistics" },
    subtitle: { uz: "Buyurtmalar, yetkazish va haydovchilar", ru: "Заказы, доставка и водители", en: "Orders, delivery and drivers" },
    add: { uz: "Yangi buyurtma", ru: "Новый заказ", en: "New Order" },
    name: { uz: "Buyurtma", ru: "Заказ", en: "Order" },
    destination: { uz: "Manzil", ru: "Направление", en: "Destination" },
    driver: { uz: "Haydovchi", ru: "Водитель", en: "Driver" },
    weight: { uz: "Og'irlik (kg)", ru: "Вес (кг)", en: "Weight (kg)" },
  ...
  const handleAdd = async () => {
    ...
    await supabase.from("deals").insert({ name: form.name, company: form.destination || "—", contact_name: form.driver, amount: Number(form.weight) || 0, stage: "lead", user_id: user.id });
    setForm({ name: "", destination: "", driver: "", weight: "" });
    setShowAdd(false);
    fetchData();
  };

  const filtered = orders.filter(i => i.name.toLowerCase().includes(search.toLowerCase()));
  const inTransit = orders.filter(i => !["won", "lost"].includes(i.stage)).length;
  const delivered = orders.filter(i => i.stage === "won").length;
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[{ label: t("total"), value: orders.length, icon: Package }, { label: t("inTransit"), value: inTransit, icon: Truck }, { label: t("delivered"), value: delivered, icon: MapPin }].map(s => (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-lg forge-shadow-sm overflow-hidden">
        <table className="w-full"><thead><tr className="border-b border-border">
          ...
          <th className="text-left text-xs font-medium text-muted-foreground px-5 py-3">{t("destination")}</th>
          <th className="text-left text-xs font-medium text-muted-foreground px-5 py-3">{t("driver")}</th>
        ...
        <tbody className="divide-y divide-border">
          ...
          filtered.map(i => (
            <tr key={i.id} className="forge-transition forge-hover group">
              ...
              <td className="px-5 py-3.5 text-sm text-foreground">{i.contact_name || "—"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onClick={e => e.stopPropagation()} className="bg-card border border-border rounded-lg forge-shadow-md w-full max-w-md p-5">
            ...
            <div className="space-y-3">
              {([["name","name"],["destination","destination"],["driver","driver"],["weight","weight"]] as const).map(([k,f])=>(
