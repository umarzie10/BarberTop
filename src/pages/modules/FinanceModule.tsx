...
import { Plus, Search, Landmark, CreditCard, TrendingUp, Users, MoreHorizontal } from "lucide-react";
...
const FinanceModule = () => {
  ...
  const [form, setForm] = useState({ name: "", company: "", amount: "", type: "" });

  const l = {
    title: { uz: "Moliya", ru: "Финансы", en: "Finance" },
    subtitle: { uz: "Mijozlar, kreditlar va to'lovlar", ru: "Клиенты, кредиты и платежи", en: "Clients, credits and payments" },
    add: { uz: "Yangi hisob", ru: "Новый счёт", en: "New Account" },
    name: { uz: "Mijoz", ru: "Клиент", en: "Client" },
    ...
    total: { uz: "Jami hisoblar", ru: "Всего счетов", en: "Total Accounts" },
    active: { uz: "Faol", ru: "Активные", en: "Active" },
    closed: { uz: "Yopilgan", ru: "Закрытые", en: "Closed" },
    search: { uz: "Qidirish...", ru: "Поиск...", en: "Search..." },
    save: { uz: "Saqlash", ru: "Сохранить", en: "Save" },
    cancel: { uz: "Bekor", ru: "Отмена", en: "Cancel" },
    empty: { uz: "Hali hisoblar yo'q", ru: "Пока нет счетов", en: "No accounts yet" },
  };
  ...
  const handleAdd = async () => {
    ...
    await supabase.from("deals").insert({ name: form.name, company: form.company || "—", amount: Number(form.amount) || 0, contact_name: form.type, stage: "lead", user_id: user.id });
    setForm({ name: "", company: "", amount: "", type: "" });
    setShowAdd(false);
    fetchData();
  };

  const filtered = accounts.filter(i => i.name.toLowerCase().includes(search.toLowerCase()));
  const active = accounts.filter(i => !["won","lost"].includes(i.stage)).length;
  const closed = accounts.filter(i => i.stage === "won").length;
  const totalAmount = accounts.reduce((s, d) => s + Number(d.amount), 0);
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[{ label: t("total"), value: String(accounts.length), icon: Landmark }, { label: t("active"), value: String(active), icon: TrendingUp }, { label: t("closed"), value: String(closed), icon: Users }, { label: t("amount"), value: `$${totalAmount.toLocaleString()}`, icon: CreditCard }].map(s => (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-lg forge-shadow-sm overflow-hidden">
        <table className="w-full"><thead><tr className="border-b border-border">
          ...
          <th className="text-left text-xs font-medium text-muted-foreground px-5 py-3">{t("company")}</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onClick={e => e.stopPropagation()} className="bg-card border border-border rounded-lg forge-shadow-md w-full max-w-md p-5">
            ...
            <div className="space-y-3">
              {([["name","name"],["company","company"],["amount","amount"],["type","type"]] as const).map(([k,f])=>(
